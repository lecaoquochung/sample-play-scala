...
By default the server uses port 9001, but a custom port can be given as
...
class DemoServer(ThreadingMixIn, HTTPServer):
    ...
    def serve(self, directory=ROOT):
        chdir(directory)
        print('Sample login server starting on port %d.' % self.server_address[1])
        try:
            server.serve_forever()
        except KeyboardInterrupt:
            server.server_close()
        print('Sample login server stopped.')
